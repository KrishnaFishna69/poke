...
logging.basicConfig(
    ...
    handlers=[
        logging.StreamHandler(sys.stdout)  # Output to GitHub Actions logs
    ]
)
...
def main():
    ...
    logging.info("🚀 Starting daily price update for GitHub Actions")
    logging.info(f"Date: {today}")
    logging.info(f"Price column: {price_column}")
    
    # Check if database exists
    if not os.path.exists("pokemon_cards.db"):
        logging.error("❌ Database file not found! Please ensure pokemon_cards.db exists.")
        sys.exit(1)
                    ...
                    try:
                        ...
                        if update_card_price_in_database(card_id, market_price, price_column):
                            ...
                            if market_price is not None:
                                logging.info(f"Updated: {safe_str(card.name)} - Price: ${market_price:.2f}")
                            else:
                                logging.debug(f"Updated: {safe_str(card.name)} - No price available")
    ...
    logging.info(f"Cards with updated prices: {cards_updated}")
    logging.info(f"New price column: {price_column}")
    ...
    # Exit with success
    logging.info("🎉 GitHub Actions price update completed successfully!")
    return 0
